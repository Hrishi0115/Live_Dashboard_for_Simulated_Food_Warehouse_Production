...
active_packages = []
package_count = 0  # Counter for packages passing the sensor
...
while running:
    ...
    for package in active_packages[:]:
        ...
        if package['pos'][0] + package['size'][0] > sensor_position and not 'counted' in package:
            package['counted'] = True
            package_count += 1  # Increment count when package passes the sensor
...
print(f"Total packages counted: {package_count}")
pygame.quit()
sys.exit()
